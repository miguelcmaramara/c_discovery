...
#include <signal.h>
#include <stdlib.h>
#include <string.h> // for strcmp macro
#include <unistd.h>
#include <fcntl.h>  // for open macro
#include <stdarg.h>    // for variable argument lists
#include <getopt.h> // for parsing command line args
/* shut lint up about the following when return value ignored */

#define Signal (void)signal     // typecasts the non-void functions as void so
#define Unlink (void)unlink     // so for piece of mind with the linter
...
/*
 * Creates gets switches in the command line
 */
...
void plan_b(char * filename) {
    ...
    // return TRUE; // TODO: delete placeholder
}
...
void set_signals()
{
    ...
    if (signal(SIGHUP, SIG_IGN) != SIG_IGN)
        Signal(SIGHUP, (void (*)(int)) my_exit);
    /*NOSTRICT*/
    if (signal(SIGINT, SIG_IGN) != SIG_IGN)
        Signal(SIGINT, (void (*)(int)) my_exit);
}
...
void get_some_switches() {
    ...
    while(TRUE){
        ...
        if(*Argv[optind] == '-' && *(Argv[optind] + 1) == 'x'){
#ifdef DEBUGGING
            debug = atoi(Argv[optind] + 2);
            optind += 1;
#else
        ...
        // if(c == 'x'){
            // c = getopt_long(Argc, Argv, "+b:cd:D:elno:pr:Rsx", options, &option_index);
            // continue;
        // }

        c = getopt_long(Argc, Argv, "+b:cd:D:elno:pr:Rsx", options, &option_index);

        if(c < 0 && Argv[optind] == 0/*optind== Argc*/)
            return;

        if(c == -1 && (*Argv[optind] == '+' && *(Argv[optind] + 1) == 0)){
            optind++;
            return;     // '+' was found
        }


        if (c == -1 && (*Argv[optind] != '-' || !*(Argv[optind] + 1))) {
        // if (*s != '-' || !s[1]) {
            if (filec == MAXFILEC)
                fatal("Too many file arguments.\n");
            filearg[filec++] = savestr(Argv[optind++]);  // handles larger args
            if(optind == Argc)
                return;
            else 
                continue;
...
int orig_main(int argc, char **argv) {
    ...
    setbuf(stderr,serrbuf);         // Creates custom buffer for stderr
    for (i = 0; i<MAXFILEC; i++)    
        filearg[i] = Nullch;    // Creates Null char file pointer for each file
    Mktemp(TMPOUTNAME);     // Creates temporary filenames TODO: figure out why
                            // this is an issue
    Mktemp(TMPINNAME);
    Mktemp(TMPREJNAME);
    Mktemp(TMPPATNAME);
    ...
    /* parse switches */
    Argc = argc;    // swithes are the data files
    Argv = argv;
    get_some_switches();
